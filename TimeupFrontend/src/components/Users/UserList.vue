...
const deleteUserEvent = async (idUserToDelete, user) => {
  ...
  userSignInRes.deletedUser(idUserToDelete, user);
};
</script>
...
<template>
  ...
  <div class="bg-white rounded-xl mt-[120px] m-auto w-[1200px] h-[850px]">
    <div class="grid grid-flow-row grid-flow-col bg-white rounded-xl border m-auto w-full">
        ...
        <div class="relative w-[90%] m-auto">
          <form>
            <input
              ...
              type="text" />
            <svg class="placeholder" width="2em" height="2em" viewBox="0 0 28 34">
    ...
    <div class="grid grid-flow-row grid-cols-1">
      ...
      <div v-else class="scroller w-full h-[300px] bg-[#E9E9E9]">
        <div
          ...
          v-for="(userList, index) in userLists" :key="index">
          ...
          <div class="row-start-1 col-start-10 col-span-1">
            <router-link :to="{name: 'UserDetail' ,params:{iduser:userList.idUser}}">
              <!-- <router-link
            ...
            > -->
              <button class="hover:bg-[#D9D9D9] rounded-sm">
                <svg class="hover:text-[#008EDE]" width="2em" height="2em" viewBox="0 0 24 24">
                  <path fill="currentColor"
          ...
          <div class="row-start-1 col-start-12 col-span-1">
            <button @click="changeDeleteDialogShow(userList)" class="hover:bg-[#D9D9D9] rounded-sm">
              <svg class="hover:text-[#F24052]" width="2em" height="2em" viewBox="0 0 24 24">
                <path fill="currentColor"
          ...
          <DeleteUserDialog v-if="userList.deleteDialogStatus" :propsUserToDelete="userList"
            @cancelDeleteUser="changeDeleteDialogClose(userList)" @confirmDeleteUser="deleteUserEvent" />
        </div>
      </div>
    </div>
  </div>
