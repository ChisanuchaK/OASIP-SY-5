  ...
  >
    <div class="">
      <div class="relative bg-white rounded-lg text-xl p-5 px-12">
        ...
        <img
          class="absolute top-0 right-0 text-4xl text-lg font-normal h-[30px] w-[30px] mt-3 mr-7 cursor-pointer"
          @click="$emit('onCloseDetails')"
        ...
        <div class="grid grid-flow-row grid-cols-2 p-1 gap-1 pt-5 text-center">
          ...
          <div class="row-start-8 col-start-1 col-end-3 col-span-2">
            <textarea
              class="bg-gray-200 w-full resize-none rounded text-center bg-"
              placeholder="No Message"
              disabled
              v-model="bookingDetail.eventNotes"
              name=""
              id=""
              cols="90"
              rows="5"
          ...
          <DeleteBookingDialog
            v-if="bookingDetail.statusClickDelete"
            :bookingsFromDetails="booking"
            :bookingsFromLoopBookings="booking"
            @onCancelDelete="changeDeleteDialog(bookingDetail)"
            @onConfirmDelete="removeBookingEvent"
          />

          <EditBookingDialog
            v-if="bookingDetail.statusClickEdit"
            :loopEdit="booking"
            :bookingLists="bookingLists"
            :bookingsDetailsEdit="bookingDetail"
            @EditbookingId="getEditIdFromEdit"
          />
        </div>
      </div>
    </div>
