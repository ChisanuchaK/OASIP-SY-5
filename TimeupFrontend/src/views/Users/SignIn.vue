...
import { getAllUsers,loginToUse } from '../../stores/book.js';
// let {params} = useRoute()
// console.log(params)
...
const confirmToLogin = async(dataForLogin) => {
  ...
  if ((loginData.email.trim() == '' || loginData.email.length == 0) || (!loginData.email.match(regexEmail)) || loginData.email.length > 50 ||
    loginData.password.length == 0 || (loginData.password.length < 8 && loginData.password.length > 0)
  ) {
...
<template>
  <div>
    ...
    <div class="bg-white rounded-xl mt-[100px] m-auto w-[1200px] h-[800px] border border-[#CFD8DC] drop-shadow-lg ">
      <div class="grid grid-rows-8 grid-cols-12">
        ...
        <div class="row-start-4 col-start-4 col-end-10">
          <form>
            ...
            <div class="relative m-auto w-[90%]">
              <input :style="{
                  'border-color':
                    inputEmailIsEmpty || inputEmailIsInvalid || inputEmailIsOver ? 'red' : ''
              ...
              <label :style="{
                  'color':
                    inputEmailIsEmpty || inputEmailIsInvalid || inputEmailIsOver ? 'red' : ''
                }" class="placeholder text-[#D9D9D9]">Email</label>
        ...
        <div class="relative grid row-start-7 col-start-7 col-end-10 w-full content-center">
          <!-- <router-link class="w-[70%] m-auto" :to="{ name: '' }"> -->
            <button @click="confirmToLogin(loginData)"
              class="w-[70%] m-auto rounded-md bg-[#105E99] text-[#ffffff] w-full m-auto p-2 hover:bg-[#004980] transition delay-75">
              Sign-in
            </button>
          <!-- </router-link> -->
          <label v-if=" isInvalid ||inputEmailIsEmpty || inputEmailIsInvalid || inputEmailIsOver || inputPasswordIsEmpty || inputPasswordIsInvalid"  class="left-[40px] absolute text-red-500 top-[70px] text-[14px] ">
            *something in form is invalid.
          </label>
        </div>

        <div class="row-start-1 row-end-7 col-start-1 bg-[#F24052] rounded-tl-lg"></div>
        <div class="row-start-1 row-end-7 col-start-12 bg-[#105E99] rounded-tr-lg"></div>
      </div>
    </div>
  </div>
</template>
